...
import (
	...
	"database/sql/driver"
	"fmt"
	"github.com/jifuy/commongo/dbClient/dm/parser"
...
func (conn *DmConnection) setTrxFinish(status int32) {
	switch status & Dm_build_392 {
	case Dm_build_389, Dm_build_390, Dm_build_391:
		conn.trxFinish = true
	default:
...
func (dc *DmConnection) executeInner(query string, execType int16) (interface{}, error) {
	...
	if execType == Dm_build_356 && dc.dmConnector.enRsCache {
	...
	if optParamList != nil && len(optParamList) > 0 {
		info, err = dc.Access.Dm_build_81(stmt, optParamList)
		if err != nil {
			stmt.nativeSql = query
			info, err = dc.Access.Dm_build_87(stmt, execType)
		}
	} else {
		info, err = dc.Access.Dm_build_87(stmt, execType)
	}
...
func (dc *DmConnection) reconnect() error {
	...
	for _, stmt := range dc.stmtMap {
		err = dc.Access.Dm_build_59(stmt)
		if err != nil {
			return err
		}
...
func (dc *DmConnection) exec(query string, args []driver.Value) (*DmResult, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, Dm_build_357)
		if err != nil {
			return nil, err
		}
...
func (dc *DmConnection) query(query string, args []driver.Value) (*DmRows, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, Dm_build_356)
		if err != nil {
			return nil, err
		}
...
func (dc *DmConnection) driverQuery(query string) (*DmStatement, *DmRows, error) {
	...
	stmt.innerUsed = true
	stmt.innerExec = true
	info, err := dc.Access.Dm_build_87(stmt, Dm_build_356)
	if err != nil {
		return nil, nil, err
	}
	dc.lastExecInfo = info
